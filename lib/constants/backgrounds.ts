...
// Build a mapping from Cloudinary public ID -> local public path (if available).
// This lets the app fall back to local `/...` assets during dev when Cloudinary
// is not configured. Keys match the public IDs used in `cloudinaryPublicIds`.
const publicIdToLocalPath: Record<string, string> = {};
Object.keys((globalThis as any).__BACKGROUND_CATEGORIES_FOR_BUILD__ || {}).length; // noop to hint about SSR-safe usage
Object.entries((() => {
  try {
    return (require('./cloudinary-backgrounds').backgroundCategories) as Record<string, string[]>;
  } catch (e) {
    // In some build contexts `require` may not be available; fall back to import above
    return ({} as Record<string, string[]>);
  }
})()).forEach(([category, files]) => {
  files.forEach((filePath) => {
    // filePath looks like '/mac/mac-asset-8.jpg' -> build key 'backgrounds/backgrounds/mac/mac-asset-8'
    const withoutLeading = filePath.replace(/^\//, '');
    const withoutExt = withoutLeading.replace(/\.[^/.]+$/, '');
    const key = `backgrounds/backgrounds/${withoutExt}`;
    publicIdToLocalPath[key] = filePath;
  });
});
      ...
      if (isCloudinaryPublicId) {
        try {
          ...
          if (cloudName) {
            ...
            // Try to map the Cloudinary public ID to a local `/public` path.
            const local = publicIdToLocalPath[value as string];
            if (local) {
              imageUrl = local;
            } else {
