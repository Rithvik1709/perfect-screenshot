  ...
  try {
    ...
    for (const overlay of imageOverlays) {
      ...
      if (typeof overlay.src === 'string' && !overlay.isCustom) {
        // Treat leading-slash paths, data URLs and absolute URLs as local/external resources.
        // Only treat values as Cloudinary public IDs when they are plain ids (no leading slash, no protocol)
        const srcStr = overlay.src as string;
        const isLikelyLocalOrExternal = srcStr.startsWith('/') || srcStr.startsWith('http') || srcStr.startsWith('data:');
        const isCloudinaryId = !isLikelyLocalOrExternal;
