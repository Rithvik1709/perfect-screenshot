...
import { getCldImageUrl, getCloudinaryConfig } from '@/lib/cloudinary';
                          ...
                          if (isCloudinaryPublicId) {
                            ...
                            if (cloudName) {
                              try {
                                ...
                                console.warn('getCldImageUrl failed, falling back to local path', e);
                                imageUrl = '/' + String(backgroundConfig.value).replace(/^backgrounds\/backgrounds\//, '');
                              }
                            } else {
                              // No Cloudinary configured - fall back to a local public path when possible
                              imageUrl = '/' + String(backgroundConfig.value).replace(/^backgrounds\/backgrounds\//, '');
                            }
                          }
                          // If it's a local path (starts with '/'), use it directly as imageUrl
                          if (typeof backgroundConfig.value === 'string' && backgroundConfig.value.startsWith('/')) {
                            imageUrl = backgroundConfig.value as string;
                          }
                              ...
                              <div key={category} className="space-y-2">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                  {categoryBackgrounds.map((publicId: string, idx: number) => {
                                      const thumbnailUrl = publicId && typeof publicId === 'string' && publicId.startsWith('/')
                                        ? publicId
                                        : (() => {
                                            const cloudName = getCloudinaryConfig().cloudName;
                                            if (cloudName) {
                                              try {
                                                ...
                                                console.warn('getCldImageUrl failed for thumbnail, falling back to local path', e);
                                                return '/' + String(publicId).replace(/^backgrounds\/backgrounds\//, '');
                                              }
                                            }
                                            // No cloud configured - try local path derived from publicId
                                            return '/' + String(publicId).replace(/^backgrounds\/backgrounds\//, '');
                                          })();
