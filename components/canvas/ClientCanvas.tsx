...
function CanvasRenderer({ image }: { image: HTMLImageElement }) {
  ...
  useEffect(() => {
    if (backgroundConfig.type === 'image' && backgroundConfig.value) {
      ...
      // We'll try the primary URL first and then fall back to a few likely local paths
      ...
      if (typeof imageUrl === 'string' && !imageUrl.startsWith('http') && !imageUrl.startsWith('blob:') && !imageUrl.startsWith('data:')) {
        // If it starts with '/', treat it as a local public path and use as-is
        if (imageUrl.startsWith('/')) {
          // local path (e.g. /assets/asset-20.jpg) - leave imageUrl unchanged
        } else {
          // It might be a Cloudinary public ID, construct URL if it exists in the mapping
          const backgroundsModule = require('@/lib/cloudinary-backgrounds')
          const cloudinaryPublicIds = backgroundsModule.cloudinaryPublicIds || []

          if (cloudinaryPublicIds.includes(imageUrl)) {
            // Check whether Cloudinary is configured. If not, try to map the
            // public ID to a local `/public/...` path so dev works without Cloudinary.
            ...
            if (cloudName) {
              // Use container dimensions for better quality when Cloudinary is available
              ...
              // Build a mapping from public ID -> local path using backgroundCategories
              const backgroundCategories = backgroundsModule.backgroundCategories || {}
              const publicIdToLocal: Record<string, string> = {}
              Object.entries(backgroundCategories).forEach(([cat, files]: any) => {
                files.forEach((fp: string) => {
                  const withoutLeading = fp.replace(/^\//, '')
                  const withoutExt = withoutLeading.replace(/\.[^/.]+$/, '')
                  const key = `backgrounds/backgrounds/${withoutExt}`
                  publicIdToLocal[key] = fp
                })
              })

              const mapped = publicIdToLocal[imageUrl as string]
              if (mapped) {
                imageUrl = mapped
              } else {
      ...
      // Build fallback list for common local paths/extensions.
      // Prefer building fallbacks from the resolved imageUrl (which may already
      // be a mapped local path). This ensures we try the actual `/public/...`
      // file first, then sensible alternates.
      const buildFallbacks = (orig: string): string[] => {
        if (typeof orig !== 'string') return []
        const candidates: string[] = []

        const value = orig.replace(/^\//, '')
        const parts = value.split('/')
        const base = parts[parts.length - 1]
        const hasExt = /\.[^/.]+$/.test(base)

        // If orig appears to be a local path (starts with '/'), try it first
        if (orig.startsWith('/')) {
          candidates.push(orig)
          if (!hasExt) {
            candidates.push(`/${value}.avif`)
            candidates.push(`/${value}.webp`)
            candidates.push(`/${value}.jpg`)
            candidates.push(`/${value}.png`)
          }
        } else {
          // Orig might be a public-id like backgrounds/backgrounds/radiant/radiant7
          // Try direct path under backgrounds, then common public folders
          candidates.push(`/backgrounds/${base}.avif`)
          candidates.push(`/backgrounds/${base}.webp`)
          candidates.push(`/backgrounds/${base}.jpg`)
          candidates.push(`/backgrounds/${base}.png`)

          candidates.push(`/assets/${base}.avif`)
          candidates.push(`/assets/${base}.webp`)
          candidates.push(`/assets/${base}.jpg`)
          candidates.push(`/assets/${base}.png`)
        }

        return Array.from(new Set(candidates))
      }

      // Build fallbacks from the resolved imageUrl (mapped local path if available),
      // otherwise fall back to the original imageValue.
      const fallbacks = buildFallbacks(imageUrl as string)
      let fallbackIndex = 0
      ...
      img.onerror = () => {
        ...
        if (fallbackIndex < fallbacks.length) {
          ...
          console.warn(`Background image failed, trying fallback: ${next}`)
          img.src = next
          return
        }
      ...
      // Start with the computed imageUrl (may be a Cloudinary URL or original value)
      img.src = imageUrl
    } else {
      setBgImage(null)
    }
    ...
    const loadOverlays = async () => {
      ...
      for (const overlay of imageOverlays) {
        ...
        try {
          ...
          // If overlay.src starts with '/', it's a local path in /public and should be used directly
          const imageUrl = (typeof overlay.src === 'string' && overlay.src.startsWith('/'))
            ? overlay.src
